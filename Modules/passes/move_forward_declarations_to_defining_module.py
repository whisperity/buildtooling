...
def main(MODULE_MAP,
  ...
  for file in tqdm(FORWARD_DECLARATIONS,
    ...
    if len(modules_of_fwding_file) > 1:
      tqdm.write("ERROR: Forward declaring file %s belongs to multiple "
                 "modules: %s. The forward declaration cannot be used as a "
                 "link of module merging."
    ...
    for line, symbol in FORWARD_DECLARATIONS[file]:
      ...
      if not modules_of_definition:
        tqdm.write("Symbol '%s' forward declared in %s was not found in the "
                   "loaded definition symbol table."
                   % (symbol, file),
                   file=sys.stderr)
        continue
      if len(modules_of_definition) > 1:
        tqdm.write("ERROR: Symbol '%s' forward declared in file %s is defined "
                   "in multiple modules: %s (by files %s). The forward "
                   "declaration cannot be used as a link of module merging."
                   % (symbol, file,
      ...
      # the link.
      # tqdm.write("Symbol '%s' forward declared in file %s in module %s "
      #            "refers to definition in %s in module %s. Setting up "
      #            "modules for merge."
      #            % (symbol, file, modules_of_fwding_file[0],
      #               list(DEFINITIONS[symbol])[0], modules_of_definition[0]))
  ...
  for comp in filter(lambda c: len(c) > 1,
    ...
    print("Merging modules due to forward declaration co-dependencies: %s."
          % ', '.join(sorted(comp)))
    file_moves = dict()
  ...
  print("-------- Final count of files in each modules after forward --------")
